...
<CodeBlock language="yaml" showLineNumbers>{treatment_docker_compose_yaml}</CodeBlock>
 ...
 - Line 3 defines the [Master of VALAWAI (MOV)](/docs/architecture/implementations/mov/deploy).
 - Lines 40 and 50 define the required services for MOV (RabbitMQ and MongoDB, respectively).
 - Line 60 configures the [C0 Patient Treatment UI](/docs/components/C0/patient_treatment_ui#deploy).
 - Line 70 configures the required PostgreSQL database for the UI.
 - Line 80 configures the [C1 NIT Protocol manager](/docs/components/C1/nit_protocol_manager#deploy).
 - Lines 90-120 configure the C2 Treatment Value Evaluators ([autonomy](/docs/components/C2/treatment_autonomy_valuator#deploy), 
